...
export default function App() {
  ...
  // Forecast always uses current inputs even when browsing history:
  // day tab -> today, month tab -> current month + today, year tab -> current year.
  useEffect(() => {
    const ctrl = new AbortController()
  ...
  // Loading vs error: spinner on first fetch AND on every tab/date switch while refetching.
  // N/A (red) only after fetch failed.
  ...
  const forecastKeys =
    period === 'day'
      ? [`day:${isoDay()}`]
      : period === 'month'
        ? [`day:${isoDay()}`, `month:${isoMonth()}`]
        : [`year:${isoYear()}`]
  const isForecastFetching = forecastKeys.some((k) => (pending[k] ?? 0) > 0)
  const isForecastInputsLoading =
    isForecastFetching ||
    (period === 'day' ? dayData == null : period === 'month' ? monthData == null || dayData == null : yearData == null)
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      <div className="max-w-2xl mx-auto px-4 pb-12 flex flex-col gap-4">
        ...
        <ForecastCard
          ...
          todayNetPhp={dayData?.cost.net_php ?? DEFAULT_COST.net_php}
          ...
          yearSolarKwh={yearData?.energy.generated_kwh ?? DEFAULT_ENERGY.generated_kwh}
          yearNetPhp={yearData?.cost.net_php ?? DEFAULT_COST.net_php}

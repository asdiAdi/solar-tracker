...
export default function CalendarModal({
  mode, day, month, year, onPickDay, onPickMonth, onPickYear, onClose,
}: CalendarModalProps) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={mode === 'day' ? 'Pick a day' : mode === 'month' ? 'Pick a month' : 'Pick a year'}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <span className="text-base font-bold">
            {mode === 'day' ? 'Select day' : mode === 'month' ? 'Select month' : 'Select year'}
          </span>
          <button
        ...
        {mode === 'day' && (
          <DayGrid selected={day} onPick={(v) => { onPickDay(v); onClose() }} />
        )}
        {mode === 'month' && (
          <MonthGrid selected={month} onPick={(v) => { onPickMonth(v); onClose() }} />
        )}
        {mode === 'year' && (
          <YearGrid selected={year} onPick={(v) => { onPickYear(v); onClose() }} />
        )}
      </div>
    </div>
  )
...
function DayGrid({ selected, onPick }: { selected: string; onPick: (v: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <button className="px-3 py-1.5 rounded-lg text-lg font-bold" style={navBtnStyle()} onClick={() => step(-1)} aria-label="Previous month">‹</button>
        <span className="text-base font-bold">{MONTH_NAMES[viewM - 1]} {viewY}</span>
        <button className="px-3 py-1.5 rounded-lg text-lg font-bold" style={navBtnStyle(atMaxMonth)} onClick={() => step(1)} disabled={atMaxMonth} aria-label="Next month">›</button>
      </div>
      <div className="grid grid-cols-7 gap-1">
        {WEEKDAYS.map((w) => (
          <span key={w} className="text-center text-xs font-bold" style={{ color: 'var(--muted)' }}>{w}</span>
        ))}
...
function MonthGrid({ selected, onPick }: { selected: string; onPick: (v: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <button className="px-3 py-1.5 rounded-lg text-lg font-bold" style={navBtnStyle()} onClick={() => setViewY((y) => y - 1)} aria-label="Previous year">‹</button>
...
const PAGE = 12
function YearGrid({ selected, onPick }: { selected: string; onPick: (v: string) => void }) {
  const cy = currentYear()
  const selY = Number(selected) || cy
  const [base, setBase] = useState(() => Math.min(selY, cy) - (Math.min(selY, cy) % PAGE))
  const years = Array.from({ length: PAGE }, (_, i) => base + i)

  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <button className="px-3 py-1.5 rounded-lg text-lg font-bold" style={navBtnStyle()} onClick={() => setBase((b) => b - PAGE)} aria-label="Previous years">‹</button>
        <span className="text-base font-bold">{base} – {Math.min(base + PAGE - 1, cy)}</span>
        <button
          className="px-3 py-1.5 rounded-lg text-lg font-bold"

...
export async function fetchDayForecast(
  ...
  try {
    ...
    const todayYieldKwh = Math.max(
      todaySolarKwh,
      Math.min(todayRad * ratio, todayCap),
    );
    const hoursElapsed =
    ...
    const billPhp =
      dayProgress > 0.02 ? netBase / Math.max(dayProgress, 0.05) : netBase;
...
export async function fetchYearForecast(
  ...
  try {
    const daily = await fetchMeteoDaily();
    const suns = (daily.sunshine_duration ?? [])
      .map((s) => s / 3600)
...
export async function fetchMonthForecast(
  ...
  const c = await fetchForecastInner(
    soFarSolarKwh,
    todaySolarKwh,
    soFarNetPhp,
    todayConsumedKwh,
...
async function fetchForecastInner(
  ...
  try {
    ...
    if (count > remaining && remaining > 0)
      forecastKwh = (forecastKwh / count) * remaining;
